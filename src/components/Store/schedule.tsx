...
const schedule: SpektRaw[] = [
    ...
    shows: [
      {
        ...
        endDate: '7 октября',
        time: '∞',
        otherText: <>Попасть на «Голос дороги» можно в любое время в течение всего фестиваля, достаточно оказаться рядом с Перфотачкой и попросить водителя отправиться в путешествие. Узнать актуальное положение тачки можно в <Link to={perfotachkaChatLink}>чатике</Link></>,
      },
    ],
  },
  {
    ...
    shows: [
      {
        ...
        endDate: '7 октября',
        time: '∞',
        otherText: <>Посмотреть путеводитель можно в любое время в течение всего фестиваля, достаточно оказаться рядом с Перфотачкой и попросить водителя открыть бардачок. Узнать актуальное положение тачки можно в <Link to={perfotachkaChatLink}>чатике</Link></>,
      },
    ],
  },
  {
  ...
  },
  {
    name: 'Timemachine: Single, Twain, Unity, Community',
    age: 16,
    short: 'Интерактивное путешествие в прошлое с помощью музыки',
    desc: 'Это интерактивное путешествие в прошлое с помощью музыки 12 временных периодов. Каждый из них отсылает нас к одному из поколений Toyota Corolla, выпускаемых с 1966 по 2018 года. В ходе перформанса мы рассмотрим, как человек взаимодействует с самим собой, другим человеком, компанией людей и обществом. Что будет: 4 диджей-сета и пластические высказывания артистов на темы каждого из сетов. Всего 12 композиций, по хиту на каждое поколение Corolla. В первых трех сетах (Single, Twain и Unity) участвуют приглашенные артисты, а к последнему (Community) могут присоединиться зрители.',
    disc: '',
    creators: 'Ксюша Коган (Ширякина), Женя Марков',
    shows: [
      {
